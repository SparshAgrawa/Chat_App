...
class ChatBox extends Component {
  ...
  render() {
    const { currentChatsList, currentContactOrGroup, showDescription } =
      this.props;
    return (
      <div className="ChatBox">
        ...
        {showDescription && (
          <Descriptions currentContactOrGroup={currentContactOrGroup} />
        )}

        <main className="chatBoxMain">
          {currentChatsList.map((chat, index) =>
            chat.isMyMessage ? (
              <SendingChat chat={chat} key={`chat-${index}`} />
            ) : (
